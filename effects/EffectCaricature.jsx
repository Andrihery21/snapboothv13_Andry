  ...
  useEffect(() => {
    const fetchEffects = async () => {
      try {
        ...
        // Récupérer les effets de type 'caricature' depuis Supabase
        ...
        const formattedEffects = data.map(effect => ({
          ...
          api_type: effect.api_type || 'lightx' // 'lightx' par défaut pour les caricatures
        }));
        
        setEffects(formattedEffects);
      } catch (err) {
        console.error('Erreur lors du chargement des effets Caricature:', err);
  ...
  if (effects.length === 0) {
    return (
      <div className="p-4 text-center text-gray-500">
        Aucun effet caricature disponible
      </div>
    );
  }
    ...
    <div className="flex flex-wrap gap-4">
      {effects.map((effect) => (
        <button
          ...
          className={`w-24 ${effect.api_type === 'aiapi' ? 'bg-purple-700 hover:bg-purple-900' : 'bg-blue-700 hover:bg-blue-900'} text-white rounded-lg overflow-hidden shadow-md`}
        >
          ...
          <div className="p-2 text-center font-medium text-sm">
            {effect.label}
            <span className="text-xs block opacity-75">{effect.api_type === 'aiapi' ? 'Aiapi' : 'LightX'}</span>
          </div>
        </button>
      ))}
    </div>
