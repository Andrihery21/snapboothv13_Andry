...
import EffectCaricature from '../../components/effects/EffectCaricature';
...
export default function EcranVerticale3Captures({ eventId }) {
  ...
  const savePhoto = async () => {
    ...
    try {
     ...
     if (response.data.error_code !== 0) {
      ...
      const newUrl = response.data.data.image_url;
      console.log("Voici l'url de l'image généré",newUrl ); 
      ...
      setTimeout(() => {
        ...
        const processedImage = imgSrc; // Dans un cas réel, ce serait l'image traitée
        setImageTraitee(processedImage);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-900 to-yellow-900 text-white relative overflow-hidden">
          ...
          {!showAdminDashboard && (
            <>
              ...
              <header className="p-4 flex justify-between items-center">
                <button 
                  onClick={retourAccueil}
                  className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-full flex items-center"
                >
                ...
                <h1 className="text-2xl font-bold text-white">Effet Caricature</h1>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-4">
                ...
                <div className="relative w-full max-w-2xl aspect-[9/16] bg-black rounded-xl overflow-hidden border-4 border-yellow-600 shadow-2xl">
                ...
                {!imgSrc && !montrerResultat && !montrerQRCode &&  (
                  <div className="mt-8 flex flex-col items-center">
                    {!montrerSelectionEffets ? (
                      <button
                        onClick={() => capture()}
                        className="bg-yellow-600 hover:bg-yellow-700 text-white text-xl font-bold py-4 px-8 rounded-full shadow-lg transform transition-transform hover:scale-105"
                      >
                        {CAPTURE_BUTTON_TEXT}
                      </button>
                    ) : (
                      <div className="bg-black/70 p-4 rounded-xl border-2 border-yellow-600 w-full max-w-2xl">
                        <h3 className="text-xl font-bold text-white mb-4 text-center">Choisissez votre style de caricature</h3>
