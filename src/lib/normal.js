...
/**
 * Applique l'effet Eclatant à une image (augmente le contraste et la saturation)
 * @param {HTMLCanvasElement|string} inputCanvas - Canvas source ou URL de l'image
 * @returns {Promise<HTMLCanvasElement>} - Canvas avec l'effet appliqué
 */
export async function applyEclatant(inputCanvas) {
  console.log("Application de l'effet Eclatant");
  ...
  try {
    ...
    // Paramètres pour l'effet Eclatant
    ...
    console.error("Erreur lors de l'application de l'effet Eclatant:", error);
    return inputCanvas; // En cas d'erreur, retourner l'image source
  }
}
